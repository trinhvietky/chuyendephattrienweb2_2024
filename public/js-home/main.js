...
(function ($) {
    ...
    document.querySelectorAll('.btn-num-product-down').forEach(function (button) {
        button.addEventListener('click', function () {
            var quantityInput = this.nextElementSibling; // Lấy input số lượng (phần tử kế tiếp)
            var numProduct = Number(quantityInput.value); // Lấy giá trị số lượng

            if (numProduct > 1) { // Kiểm tra nếu số lượng lớn hơn 1
                quantityInput.value = numProduct - 1; // Giảm số lượng
                updateTotal(this.closest('.table_row')); // Cập nhật tổng tiền cho sản phẩm này
            }
        });
    });
    ...
    document.querySelectorAll('.btn-num-product-up').forEach(function (button) {
        button.addEventListener('click', function () {
            var quantityInput = this.previousElementSibling; // Lấy input số lượng (phần tử trước)
            var numProduct = Number(quantityInput.value); // Lấy giá trị số lượng

            quantityInput.value = numProduct + 1; // Tăng số lượng
            updateTotal(this.closest('.table_row')); // Cập nhật tổng tiền cho sản phẩm này
        });
    });
